...
module netcdfmod
  ...
  integer :: NDIMS, NVAR
  character(len=13), dimension(24) :: varlist=(/ 'U            ', 'V            ', 'W            ', 'RHO          ', 'PT           ', 'PTV          ', 'Qv           ', 'Qc           ', 'Qr           ', 'RH           ', 'MSE          ', 'LWP          ', 'CT_height    ', 'Buoy         ', 'Beff         ', 'Entrainment_c', 'Detrainment_c', 'QT_tend_1    ', 'QT_tend_2    ', 'QT_tend_3    ', 'QT_tend_5    ', 'QT_tend_6    ', 'QT_tend_8    ', 'QT_tend_9    ' /)
  ...
  allocate ( state(1:NXO,1:NYO,1:NZO,1:NTO), tend(1:NXO,1:NYO,1:NZO,1:NTO), wind(1:NXO,1:NYO,1:NZO,1:NTO) )
  ...
  subroutine write_slice (t,height,tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,tmp7,tmp8,tmp9,tmp10,tmp11,tmp12,tmp13,tmp14)
  
  implicit none
  
  integer, intent(in) :: t
  real, intent(in) :: height
  real, dimension(1:NXO,1:NYO), intent(in) :: tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,tmp7,tmp8,tmp9,tmp10,tmp11,tmp12,tmp13,tmp14
  ...
  if (new) then
    ...
    call register_var ('mse  ', n, ncid, dimids, varids)
    call register_var ('ql   ', n, ncid, dimids, varids)
    call register_var ('lwp  ', n, ncid, dimids, varids)
    call register_var ('ctop ', n, ncid, dimids, varids)
    call register_var ('cid  ', n, ncid, dimids, varids)
    call register_var ('cra  ', n, ncid, dimids, varids)
    call register_var ('cmfl ', n, ncid, dimids, varids)
    call register_var ('cmse ', n, ncid, dimids, varids)
    call register_var ('cb   ', n, ncid, dimids, varids)
    call register_var ('cent ', n, ncid, dimids, varids)
    call register_var ('cdet ', n, ncid, dimids, varids)
  ...
  call check( nf90_put_var(ncid, varids(n+7), tmp7, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+8), tmp8, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+9), tmp9, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+10), tmp10, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+11), tmp11, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+12), tmp12, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+13), tmp13, start=start, count=count) )
  call check( nf90_put_var(ncid, varids(n+14), tmp14, start=start, count=count) )
  ...
  if (trim(varname) == 'U') then
    ...
    state%mse = var  
  else if (trim(varname) == 'LWP') then
    state%lwp = var  
  else if (trim(varname) == 'CT_height') then
    state%ctop = var  
  else if (trim(varname) == 'PT') then
    state%pt = var  
  else if (trim(varname) == 'RHO') then
    state%dens = var  
  ...
  contains
	...
	if (present(info)) info = 'Resolved buoyancy flux'
      case('rh')
	...
	if (present(info)) info = 'Cloud ID (integer)'
      case('cra')
        if (present(name)) name = 'Radius'
        if (present(units)) units = 'm'
	if (present(info)) info = 'Cloud radius'
      case('cmf')
        if (present(name)) name = 'cloud MF'
        if (present(units)) units = 'kg/s/m2'
	if (present(info)) info = ' Mass flux per cloud '
      case('cms')
        if (present(name)) name = 'cloud MSE'
        if (present(units)) units = 'J/kg'
	if (present(info)) info = 'Mean MSE per cloud'
      case('cb')
        if (present(name)) name = 'cloud Buoyancy'
        if (present(units)) units = 'm2/s3'
	if (present(info)) info = 'Mean buoyancy per cloud'
      case('cen')
        if (present(name)) name = 'cloud entrainment'
        if (present(units)) units = '1/s'
	if (present(info)) info = 'Mean entrainment rate per cloud'
      case('cde')
        if (present(name)) name = 'cloud detrainment'
        if (present(units)) units = '1/s'
	if (present(info)) info = 'Mean detrainment rate per cloud'
      case('uid')
