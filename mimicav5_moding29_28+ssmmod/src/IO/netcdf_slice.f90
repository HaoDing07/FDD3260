...
!
  ...
  inquire(FILE=trim(FILE_NAME), EXIST=ex)
  new = ( (new_run.and.ntime==1) .or. .not.ex )
...
!
  ...
  subroutine write_variables (ndims,ncidx,ncidy,ncidz,varidsx,varidsy,varidsz,startx,starty,startz,countx,county,countz,cnt)
    ...
    if (out_qv) call write_var ('qv   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*thermo%qv)
    if (out_qt) call write_var ('qt   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*state%qt)
    ...
    if (lmicro>0.and.out_qc) call write_var ('qc   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(drop)%q)
    if (lmicro>0.and.out_qr) call write_var ('qr   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(rain)%q)
    if (lmicro>1.and.out_qi) call write_var ('qi   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(ice)%q)
    if (lmicro>2.and.out_qg) call write_var ('qg   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(grau)%q)
    if (lmicro>2.and.out_qs) call write_var ('qs   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(snow)%q)
    if (lmicro>3.and.out_qh) call write_var ('qh   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(hail)%q)
    if (lmicro>0.and.out_nc) call write_var ('nc   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(drop)%n)
    if (lmicro>0.and.out_nr) call write_var ('nr   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(rain)%n)
    if (lmicro>1.and.out_ni) call write_var ('ni   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(ice)%n)
    if (lmicro>2.and.out_ng) call write_var ('ng   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(grau)%n)
    if (lmicro>2.and.out_ns) call write_var ('ns   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(snow)%n)
    if (lmicro>3.and.out_nh) call write_var ('nh   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*hydromtr(hail)%n)
    ...
    if (lmicro>0.and.out_vp) call write_var ('vdrop   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, hydromtr(drop)%vp)
    ...
    if (out_ccn) call write_var ('ccn  ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*nuc%ccn)
    if (out_in) call write_var ('in   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*nuc%in)
...
!
      if (out_qv) call cal_fluxx (pressure%dens*thermo%qv, pressure%dens*thermo%qv, tmp1)  
    ...
    if (out_aero) then
      do h = 1, nmode
        call write_var ('an   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*aero3d(h)%n)
        call write_var ('am   ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*aero3d(h)%m)
        call write_var ('ama  ', nvar, ncidx, ncidy, ncidz, varidsx, varidsy, varidsz, startx, starty, startz, countx, county, countz, cnt, pressure%dens*aero3d(h)%ma)
      enddo
    endif
#endif    
...
!
    ...
    if (out_sst) call write_2d_var ('ssq  ', nvar, ncid, varids, start, count, cnt, pressure%dens(:,:,1)*thermo%qv(:,:,1), lhov)
...
9100 format(11x,"Max & Min ",a5,"          ",f13.4,f16.4)
  ...
  subroutine check(status,name)
    ...
    if(status /= nf90_noerr) then 
      ...
      print *, 'AN ERROR WAS DETECTED IN NETCDF SLICE, FILE/VARIABLE: ', trim(name)
