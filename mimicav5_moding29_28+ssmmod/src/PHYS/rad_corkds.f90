...
module ckd
  ...
  use gridno, only : datadir
  use radia, only : mair, nv, nv1, mb, mbs, mbir, totalpower
  use shared_data, only : mypid
   implicit none
  private
...
contains
  ...
  subroutine init_ckd
    ...
    OPEN ( unit = 66, file = datadir(1:len_trim(datadir))//'/ckd.dat', status = 'old' )
    ...
    mbir = mb - mbs
    !if (mypid==0) print 600, datadir(1:len_trim(datadir))//'/ckd.dat', ngases, mb, mbs, mbir, sum(band%power)
    ...
    do n=1,ngases
       ...
       !   if (mypid==0) print 601, gas(n)%name, gas(n)%iband, gas(n)%noverlap,              &
    ...
    !do ib=1,mb
       !if (mypid==0) print 602, ib, band(ib)%power, band(ib)%llimit, band(ib)%rlimit,    &
       !     band(ib)%ngases, band(ib)%kg

      !if (mypid==0) then
      !print *,"Solar bands"
      !print 602, ib, solar_bands(ib)%power, solar_bands(ib)%llimit, solar_bands(ib)%rlimit,    &
      !solar_bands(ib)%ngases, solar_bands(ib)%kg     
      !print *,"Ir bands"
      !print 602, ib, ir_bands(ib)%power, ir_bands(ib)%llimit, ir_bands(ib)%rlimit,    &
      !ir_bands(ib)%ngases, ir_bands(ib)%kg 

      !endif 

    !end do
    !if (mypid==0) print 604
